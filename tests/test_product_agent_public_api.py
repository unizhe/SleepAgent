...
import sleepagent.radar_agent.product_agent as product_agent
...
PACKAGE_ROOT = (
    REPOSITORY / "sleepagent/radar_agent/product_agent/__init__.py"
)
PUBLIC_API = (
    "AgentId",
    "AuthenticatedBinding",
...
def test_package_root_uses_only_explicit_owner_imports() -> None:
    ...
    assert {node.module for node in imports} == {
        ...
        "sleepagent.radar_agent.product_agent.contracts",
        "sleepagent.radar_agent.product_agent.runner",
        "sleepagent.radar_agent.product_agent.runtime_contracts",
    }
    ...
    for path in REPOSITORY.rglob("*.py"):
        ...
        for node in ast.walk(tree):
            if (
                ...
                and node.module == "sleepagent.radar_agent.product_agent"
            ):
