...
def test_foundation_migration_is_additive_and_postgres_compatible() -> None:
    foundation_version = "010_sleep_domain_foundation"
    assert MIGRATION_VERSION == "020_legacy_authority_cutover"
    assert MIGRATION_VERSIONS[-1] == MIGRATION_VERSION
    sql = RADAR_AGENT_POSTGRES_MIGRATIONS[foundation_version]
    required_tables = {
    ...
    assert len(split_sql_statements(sql)) == 29


def test_postgres_runner_applies_function_and_triggers_as_complete_statements(
    ...
    class RecordingCursor:
        ...
        def fetchall(self):
            return [(version,) for version in MIGRATION_VERSIONS[:9]]
    ...
    store = connect_postgres_store("postgresql://fixture")

    assert store.connection is connection
    assert store.dialect == "postgres"
    assert connection.commits == 1
    foundation_statements = split_sql_statements(
        RADAR_AGENT_POSTGRES_MIGRATIONS["010_sleep_domain_foundation"]
    )
    executed = connection.recording_cursor.executed
    ...
    assert function.count("BEGIN") == 1
    assert function.count("END") == 1


def test_populated_previous_version_database_upgrades_without_changing_old_data() -> None:
    connection = sqlite3.connect(":memory:")
    for version in MIGRATION_VERSIONS[:-1]:
    ...
    connection.execute(
        ...
        INSERT INTO radar_subjects (
          subject_id, display_name, timezone_name, subject_json,
    ...
    legacy_columns = connection.execute(
        "PRAGMA table_info(radar_subjects)"
    ).fetchall()
    connection.commit()
    ...
    assert connection.execute(
        "SELECT display_name FROM radar_subjects WHERE subject_id = ?",
        ("legacy-subject",),
    ).fetchone() == ("Legacy Elder",)
    assert connection.execute(
        "PRAGMA table_info(radar_subjects)"
    ).fetchall() == legacy_columns
    assert connection.execute(
        "SELECT COUNT(*) FROM sleep_domain_raw_inbox"
    ).fetchone() == (0,)
    assert connection.execute(
        "SELECT version FROM radar_agent_schema_migrations WHERE version = ?",
        (MIGRATION_VERSION,),
    ).fetchone() == (MIGRATION_VERSION,)
