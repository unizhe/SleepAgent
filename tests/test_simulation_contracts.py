...
from simulation_verifier import (
    load_action_timeline,
    load_expected_oracle,
)
from simulation_verifier.contracts import ExpectedOracle, SimulationAction
...
def test_runtime_distribution_excludes_verifier_oracle_package() -> None:
    ...
    assert not any(
        ...
        for path in (Path(__file__).parents[1] / "sleepagent").rglob("*.py")
    )
