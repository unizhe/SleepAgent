...
def test_020_populated_migration_is_additive_and_preserves_summary() -> None:
    ...
    connection.execute(
        ...
        ) VALUES ('device-1', 'elder-1', 'legacy', 'unknown', '{}', ?, ?)
        """,
        (NOW.isoformat(), NOW.isoformat()),
    )
    connection.execute(
        ...
        INSERT INTO radar_night_summaries (
          ...
          'device-1:2026-07-29', 'device-1', 'elder-1',
          '2026-07-29', 0.5, '{"legacy":true}', ?
        )
        """,
        (NOW.isoformat(),),
    )
    ...
    ).fetchone() == ('{"legacy":true}',)
    columns = {
        row[1] for row in connection.execute("PRAGMA table_info(radar_night_summaries)")
    }
    assert {
    ...
    assert connection.execute(
        "SELECT COUNT(*) FROM radar_agent_schema_migrations"
    ).fetchone() == (len(MIGRATION_VERSIONS),)
    sql = RADAR_AGENT_POSTGRES_MIGRATIONS[MIGRATION_VERSION].upper()
