...
def test_product_runtime_is_a_top_level_namespace_without_historical_shadow() -> None:
    package_root = Path(__file__).parents[2] / "sleepagent"

    assert (package_root / "product_runtime" / "agents").is_dir()
    assert (package_root / "product_runtime" / "runner.py").is_file()
    assert not (package_root / "radar_agent").exists()
...
def test_authoritative_architecture_plan_closes_the_roster() -> None:
    plan = (
        Path(__file__).parents[2] / "docs" / "audits" / "agent-architecture" / "PLAN.md"
    ).read_text()
