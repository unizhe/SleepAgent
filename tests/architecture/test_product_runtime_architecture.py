...
import sleepagent
import sleepagent.product_runtime.deterministic_model as deterministic_model
import sleepagent.product_runtime.habit_runtime as habit_runtime
import sleepagent.product_runtime.product_persistence as product_persistence
import sleepagent.product_runtime.runner as runner_module
import sleepagent.product_runtime.runtime_contracts as runtime_contracts
import sleepagent.product_runtime.runtime_factory as runtime_factory
import sleepagent.product_runtime.runtime_ports as runtime_ports
import sleepagent.product_runtime.tooling as tooling_module
...
from tests.unit.test_product_agent_factory import build_roster
...
def test_lower_runtime_modules_do_not_import_runner_or_tooling_upward() -> None:
    ...
    assert "sleepagent.product_runtime.runner" not in persistence_imports
    ...
    assert "sleepagent.product_runtime.tooling" not in habit_imports
...
def test_runtime_factory_is_the_only_product_runner_composition_root() -> None:
    ...
    assert [path.as_posix() for path, _line in constructor_calls] == [
        "product_runtime/runtime_factory.py"
    ]
...
def test_tool_and_service_modules_do_not_import_concrete_agents() -> None:
    package_root = Path(sleepagent.__file__).resolve().parent
    product_root = package_root / "product_runtime"
    capability_paths = {
    ...
    for path in sorted(capability_paths):
        ...
        assert not any(
            module == "sleepagent.product_runtime.agents"
