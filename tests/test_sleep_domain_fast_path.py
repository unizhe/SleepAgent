...
from tests.test_night_episode_lifecycle import (
    DEVICE,
    NAMESPACE,
    _admin_policy,
    _bind,
...
def test_service_mode_allows_active_tonight_and_yesterday_followup() -> None:
    ...
    assert "sleep_domain_service_mode" not in (
        RADAR_AGENT_POSTGRES_MIGRATIONS[MIGRATION_VERSION]
    )
    connection.close()
...
def test_fast_path_migration_has_no_agent_threshold_or_service_mode_state() -> None:
    ...
    sql = RADAR_AGENT_POSTGRES_MIGRATIONS["016_deterministic_fast_path"]
    for table in (
    ...
    assert "agent_run" not in lowered
