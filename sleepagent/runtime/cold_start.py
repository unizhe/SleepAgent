...
COLD_START_POLICY_VERSION: Final = "sleepagent-cold-start.v1"
...
class NightOutOfBedBaselineValue(StrictContract):
    ...
    recent_change: Literal["stable", "increasing", "decreasing", "unknown"] = (
        "unknown"
    )
...
class ObjectiveBaselineArtifact(FrozenContract):
    """只描述客观 measurement baseline，不承载 Habit profile。"""
    ...
    @model_validator(mode="after")
    def validate_artifact(self) -> "ObjectiveBaselineArtifact":
        if self.window_start > self.window_end:
            raise ValueError("baseline window is reversed")
        if self.maturity is BaselineMaturity.UNAVAILABLE and self.value is not None:
            raise ValueError("unavailable baseline cannot carry a value")
        if self.metric_id in {"baseline.night_out_of_bed", "night_out_of_bed"} and self.value is not None:
            typed = NightOutOfBedBaselineValue.model_validate(self.value)
            object.__setattr__(self, "value", typed)
            if typed.nights_with_events > self.valid_night_count:
                raise ValueError("night-out-of-bed nights exceed valid-night count")
        return self
...
class CapabilityEligibilityReceipt(FrozenContract):
    ...
    def create(cls, **values: object) -> "CapabilityEligibilityReceipt":
        ...
        unsigned = cls.model_validate({"receipt_hash": "0" * 64, **material})
...
class MetricNightCount(FrozenContract):
    ...
    def create(cls, **values: object) -> "MetricNightCount":
        ...
        unsigned = cls.model_validate({"source_hash": "0" * 64, **material})
        return unsigned.model_copy(
            update={
...
class MetricReadinessDecision(FrozenContract):
    ...
    def create(cls, **values: object) -> "MetricReadinessDecision":
        ...
        unsigned = cls.model_validate({"decision_hash": "0" * 64, **material})
        return unsigned.model_copy(
            update={
...
def evaluate_readiness(
    *,
    ...
    if requirement.window_start is not None and requirement.window_end is not None and any(
