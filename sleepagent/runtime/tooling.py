...
class CoreProductToolService:
    ...
    def _render(
        ...
        if {
            ...
            basis_request = ProductArtifactBasisRequest.model_validate(arguments)
            if (
                not context.episode_id
                or basis_request.episode_id != context.episode_id
            ):
                raise ValueError(
                    "artifact basis does not match Product Episode"
                )
            if (
                basis_request.evidence_packet.source_scope
                != context.fact_snapshot.source_scope
            ):
                raise ValueError(
            ...
            if (
                basis_request.audience_role != snapshot.binding.role
                and "draft_material" not in scopes
            ):
                raise PermissionError(
            ...
            if (
                basis_request.audience_role == "doctor"
                and "draft_material" not in scopes
            ):
                raise PermissionError(
...
from sleepagent.runtime.registry import REGISTRY_VERSION
from sleepagent.runtime.contracts import (
    ProductToolExecutionContext,
)
from sleepagent.runtime.tools import (
