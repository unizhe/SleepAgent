...
from typing import Any, Callable, List, NoReturn, Protocol, SupportsIndex
...
class VerifiedApprovalCapability:
    ...
    def validate_exact_binding(
        ...
        *,
        ...
        if self not in _ISSUED_CAPABILITIES:
        ...
        # EXECUTING 是授权线性化点；此处复核签发时刻和哈希，确保崩溃恢复
        # 不会因墙钟过期而伪造一份新授权。
...
_ISSUED_CAPABILITIES: WeakSet[VerifiedApprovalCapability] = WeakSet()


def _create_verified_capability(grant: ApprovalGrant) -> VerifiedApprovalCapability:
    capability = object.__new__(VerifiedApprovalCapability)
    object.__setattr__(capability, "_grant", grant.model_copy(deep=True))
    _ISSUED_CAPABILITIES.add(capability)
    return capability


_CAPABILITY_ISSUER = _create_verified_capability
...
class PersistentHumanDecisionRepository:
    """Bind durable confirmation storage without leaking its schema into models."""
