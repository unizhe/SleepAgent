...
class AgentModelOutput(StrictContract):
    ...
    output_payload: CommunicationDraft | EvidencePacket | CareStrategy | SafetyDecision
...
class ProductAgentInvoker:
    ...
    def _validate_requests(agent_id: AgentId, output: AgentModelOutput) -> None:
        ...
        for tool_request in output.tool_requests:
            authorize_tool_invocation(agent_id, tool_request.tool_name)
        for collaboration_request in output.collaboration_requests:
            if collaboration_request.sender != agent_id:
                raise ValueError("collaboration sender must match invoking Agent")
            authorize_collaboration(
                collaboration_request.sender,
                collaboration_request.receiver,
                collaboration_request.request_type,
            )
