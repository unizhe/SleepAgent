# 本模块处理受围栏保护的公开命令与产品交互队列。
"""Fenced PostgreSQL handlers for command public commands and interactions."""
...
from sleepagent.persistence.migrations import (
    ...
    COMMAND_AUTHORIZATION_REVOKED_ERROR,
    COMMAND_COMMITTED_AUDIT_REASON,
    COMMAND_COMMITTED_EVENT_TYPE,
    COMMAND_INVARIANT_ERROR,
    COMMAND_INVOCATION_NAMESPACE,
    COMMAND_LEASE_LOST_ERROR,
    COMMAND_REGISTRY_VERSION,
    COMMAND_STATE_CONFLICT_ERROR,
    DETERMINISTIC_INTERACTION_MODEL_VERSION,
...
class CommandInvariantError(CommandWorkerError):
    pass


class CommandAuthorizationRevoked(CommandWorkerError):
    pass


class CommandLeaseLost(CommandWorkerError):
    pass


class CommandConflict(CommandWorkerError):
    pass
...
class CommandProcessor:
        ...
        for attempt in range(1, self.max_transaction_attempts + 1):
            try:
                with self.uow_factory.begin(scope) as uow:
                    ...
                    uow.commit()
                return result
            except (CommandWorkerError, ValueError):
                raise
                ...
                if attempt == self.max_transaction_attempts:
                    break
        raise CommandConflict("bounded command transaction retry exhausted") from last_error
    ...
    def _prepare_with_cursor(
        ...
        *,
        ...
        row = cursor.fetchone()
        if row is None:
            raise CommandLeaseLost("command operation fence was rejected")
        command_type = str(row[0])
        queue = str(row[1])
        expected_queue = _queue_for_command(command_type)
        if queue != expected_queue or queue != claim.queue:
            raise CommandInvariantError("command is routed to the wrong handler")
        required_scope = COMMAND_SCOPES.get(command_type)
        if required_scope is None:
            raise CommandInvariantError("unknown command command type")
        cursor.execute(
            f"SELECT public.{COMMAND_AUTHORITY_FUNCTION}(%s, %s, %s)",
            (claim.work_id, queue, required_scope),
        )
        authority = cursor.fetchone()
        if authority is None or authority[0] is not True:
            raise CommandAuthorizationRevoked(
        ...
        *,
        ...
        if interaction_id is not None:
            ...
            if _sha256(fact_snapshot) != str(interaction[7]):
                raise CommandInvariantError("persisted FactSnapshot integrity failed")
    ...
    def _snapshot_envelope(
        ...
        return {
            ...
            "registry_sha256": _sha256(
                {"registry": COMMAND_REGISTRY_VERSION}
            ),
            "model_sha256": _sha256(
                {"model": DETERMINISTIC_INTERACTION_MODEL_VERSION}
            ),
    ...
    def _interaction_source(
        ...
        *,
        ...
        row = cursor.fetchone()
        if row is None:
            raise CommandConflict("interaction is missing or outside this generation")
        return row
    ...
    def _validate_handle_source(
        ...
        *,
        ...
        if row is None or (
            ...
            raise CommandConflict("opaque handle is stale or outside its authority")
    ...
    def _commit_monitoring(
        ...
        *,
        ...
        if from_state != expected_from:
            raise CommandConflict("monitoring transition is not valid from current state")
        ...
        if from_version == 0:
            ...
            row = cursor.fetchone()
            if row is None:
                raise CommandConflict("monitoring snapshot CAS lost")
    ...
    def _commit_interaction(
        ...
        *,
        ...
        if command in MODEL_COMMANDS and model_response is None:
            raise CommandInvariantError("model command has no journaled response")
        ...
        raise CommandInvariantError("unsupported Product interaction command")
    ...
    def _advance_interaction(
        ...
        *,
        ...
        if str(interaction[5]) in {"confirmed", "declined", "completed", "failed"}:
            raise CommandConflict("terminal interaction cannot be advanced")
        ...
        if cursor.fetchone() is None:
            raise CommandConflict("interaction CAS lost")
        self._insert_interaction_revision(
            cursor,
            scope=scope,
            claim=claim,
    ...
    def _consume_handle(
        ...
        *,
        ...
        if cursor.fetchone() is None:
            raise CommandConflict("single-use handle consumption lost its CAS")
    ...
    def _commit_decision(
        ...
        *,
        ...
        if str(interaction[5]) != "awaiting_confirmation":
            raise CommandConflict("interaction is not awaiting confirmation")
        ...
        if cursor.fetchone() is None:
            raise CommandConflict("interaction decision CAS lost")
        self._insert_interaction_revision(
            cursor,
            scope=scope,
            claim=claim,
    ...
    def _insert_care_followup(
        ...
        *,
        ...
        if occurred_row is None:
            raise CommandInvariantError("database did not return ControlClock time")
        ...
        if prepared.interaction_id is not None:
            ...
            if str(interaction[5]) not in {"confirmed", "declined", "completed"}:
                ...
                if cursor.rowcount != 1:
                    raise CommandConflict("feedback interaction CAS lost")
                self._insert_interaction_revision(
                    cursor,
                    scope=scope,
                    claim=claim,
    ...
    def _finish_operation(
        ...
        *,
        ...
        terminal_event = {
            ...
            "event_id": event_id,
            "event_type": COMMAND_COMMITTED_EVENT_TYPE,
            "operation_id": claim.work_id,
            "command_type": prepared.command_type,
        ...
        cursor.execute(
            ...
            f"VALUES (%s, %s, %s, '{COMMAND_COMMITTED_EVENT_TYPE}', "
            "'Operation', %s, "
        ...
        if cursor.fetchone() is None:
            raise CommandLeaseLost("terminal command Operation CAS lost")
        cursor.execute(
            ...
            "%s, %s, %s, %s, %s, %s, %s, 'allow', "
            f"'{COMMAND_COMMITTED_AUDIT_REASON}', %s, %s, %s, %s, %s::jsonb, "
            "clock_timestamp())",
            (
                audit_id,
                scope.namespace_id,
...
class CommandWorkHandler:
    def __init__(
        self,
        *,
        queue: str,
        processor_factory: Callable[
            [UnitOfWorkFactory[Any]], CommandProcessor
        ] = CommandProcessor,
    ) -> None:
        if queue not in {SLEEP_COMMAND_QUEUE, PRODUCT_INTERACTION_QUEUE}:
            raise ValueError("unknown command queue")
        self.queue = queue
        self.processor_factory = processor_factory

    def __call__(self, context: WorkContext) -> WorkResult:
        try:
            scope = _command_scope(context, queue=self.queue)
            ...
            if prepared.command_type in MODEL_COMMANDS:
                ...
                invocation_key = (
                    f"{COMMAND_INVOCATION_NAMESPACE}:{prepared.operation_id}:"
        ...
        except CommandAuthorizationRevoked:
            return _terminal(COMMAND_AUTHORIZATION_REVOKED_ERROR)
        except CommandConflict:
            return _terminal(COMMAND_STATE_CONFLICT_ERROR)
        except CommandInvariantError:
            return _terminal(COMMAND_INVARIANT_ERROR)
        except (CommandLeaseLost, LeaseLostError):
            context.mark_lease_lost()
            return WorkResult(
                disposition=WorkDisposition.OUTCOME_UNKNOWN,
                error_code=COMMAND_LEASE_LOST_ERROR,
            )
        return WorkResult(
...
def build_command_worker_handlers(
    ...
    if settings.process_role != ProcessRole.WORKER:
        raise CommandInvariantError("command handlers require a worker profile")
    if (
        ...
        raise CommandInvariantError(
            "the current command implementation requires replay data with an "
            "enabled model mode"
        )
    return {queue: CommandWorkHandler(queue=queue) for queue in selected}


def _command_scope(context: WorkContext, *, queue: str) -> UowScope:
    claim = context.claim
    if (
        claim.queue != queue
        ...
        raise CommandInvariantError("claim is not exact command operation work")
    try:
        scope = context.store.uow_scope_for_claim(claim)
    except (AttributeError, TypeError, ValueError) as exc:
        raise CommandInvariantError("claim cannot form an exact UoW scope") from exc
    expected = {
    ...
    if scope.process_role != "worker" or any(
        ...
        raise CommandInvariantError("claim and command UoW scope differ")
    snapshot = claim.authorization_snapshot
    if snapshot.get("schema_version") != "authorization_snapshot.v1":
        raise CommandInvariantError("command user authorization snapshot is invalid")
    if any(
        ...
        raise CommandInvariantError("command authorization snapshot drifted")
    return scope
...
def _deterministic_model_response(
    ...
    raise CommandInvariantError("command has no deterministic model contract")
...
def _queue_for_command(command_type: str) -> str:
    ...
    raise CommandInvariantError("unknown command operation type")
...
def _json_object(value: Any, field: str) -> dict[str, Any]:
    ...
    if not isinstance(parsed, Mapping):
        raise CommandInvariantError(f"{field} must be a JSON object")
    return dict(parsed)
...
def _required_text(value: Any, field: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise CommandInvariantError(f"{field} is required")
    return value.strip()
