    ...
    def _exact_claim_query(self, kind: WorkKind) -> str:
        ...
        if kind == WorkKind.OPERATION:
            return (
                ...
                "lease_expires_at, operation_json, CASE "
                "WHEN authorization_snapshot_json IS NOT NULL THEN "
                "jsonb_build_object("
                "'schema_version', 'workload_authorization_snapshot.v1', "
                "'workload_principal_id', NULLIF(current_setting("
                "'sleepagent.service_principal_id', TRUE), ''), "
                ...
                "'data_mode', data_mode, 'run_id', run_id, 'arm_id', arm_id, "
                "'subject_id', subject_id, "
                "'purpose', NULLIF(current_setting("
                "'sleepagent.purpose', TRUE), ''), "
                "'allowed_handler', queue_name, "
                "'authorization_epoch', (authorization_snapshot_json ->> "
                "'authorization_epoch')::bigint, "
                "'privacy_epoch', (authorization_snapshot_json ->> "
                "'privacy_epoch')::bigint, "
                "'retrieval_policy_epoch', (authorization_snapshot_json ->> "
                "'retrieval_policy_epoch')::bigint) "
                "ELSE workload_authorization_snapshot_json END, "
