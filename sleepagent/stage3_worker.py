"""Stage-3 durable ScenarioClock command handler."""
...
class Stage3AdvanceError(RuntimeError):
    pass


class Stage3AdvanceLeaseLost(Stage3AdvanceError):
    pass


class Stage3ReconciliationError(RuntimeError):
    pass


class Stage3ReconciliationLeaseLost(Stage3ReconciliationError):
    pass
...
class EpisodeDateReconciliationHandler:
    ...
    def __call__(self, context: WorkContext) -> WorkResult:
        ...
        except Stage3ReconciliationLeaseLost:
        ...
        except (Stage3ReconciliationError, KeyError, TypeError, ValueError):
            return WorkResult(
        ...
        *,
        ...
        with worker_uow_factory(context).begin(scope) as uow:
            ...
            try:
                ...
                row = cursor.fetchone()
                if row is None:
                    raise Stage3ReconciliationLeaseLost(
                ...
                if any(payload[name] != value for name, value in stored.items()):
                    raise Stage3ReconciliationError(
    ...
    def __call__(self, context: WorkContext) -> WorkResult:
        try:
            ...
            if not isinstance(payload, Mapping):
                raise Stage3AdvanceError("advance payload is missing")
            seconds = payload.get("seconds")
            if type(seconds) is not int or seconds < 1 or seconds > 604_800:
                raise Stage3AdvanceError("advance seconds are invalid")
        ...
        except Stage3AdvanceLeaseLost:
            context.mark_lease_lost()
            return WorkResult(
                disposition=WorkDisposition.OUTCOME_UNKNOWN,
                error_code="demo_advance_lease_lost_reconciliation_required",
            )
        except (Stage3AdvanceError, KeyError, TypeError, ValueError):
            return WorkResult(
                disposition=WorkDisposition.TERMINAL,
    ...
    def _commit(
        ...
        *,
        ...
        with worker_uow_factory(context).begin(scope) as uow:
            ...
            try:
                cursor.execute(
                    "SELECT public.sleepagent_stage3_advance_authority_allows(%s)",
                    (claim.work_id,),
                )
                authority = cursor.fetchone()
                if authority is None or authority[0] is not True:
                    raise Stage3AdvanceError("advance authority is no longer current")
                cursor.execute(
                ...
                row = cursor.fetchone()
                if row is None:
                    raise Stage3AdvanceLeaseLost("advance operation fence was lost")
                ...
                if to_row is None:
                    raise Stage3AdvanceError("database could not advance ScenarioClock")
                to_time = to_row[0]
                cursor.execute("SELECT clock_timestamp()")
                control_row = cursor.fetchone()
                if control_row is None:
                    raise Stage3AdvanceError("database ControlClock is unavailable")
                ...
                if from_version == 0:
                    ...
                    if cursor.rowcount != 1:
                        raise Stage3AdvanceError("ScenarioClock CAS was lost")
                cursor.execute(
                ...
                if cursor.fetchone() is None:
                    raise Stage3AdvanceLeaseLost("advance terminal CAS was lost")
                cursor.execute(
                    "INSERT INTO public.backend_authorization_audit ("
                    "audit_id, namespace_id, data_mode, subject_id, principal_id, "
                    "actor_id, binding_id, decision, reason_code, policy_sha256, "
    ...
    def _release_staged_facts(
        ...
        *,
        ...
        if any(
            ...
            raise Stage3AdvanceError("staged fact canonical bytes drifted")
        if len({str(row[1]) for row in staged_rows}) != 1 or len(
            {str(row[2]) for row in staged_rows}
        ) != 1:
            raise Stage3AdvanceError("advance cannot cross journey manifests")
        cursor.execute(
        ...
        if predecessor is None or int(predecessor[2]) + 1 != items[0].sequence:
            raise Stage3AdvanceError("staged release predecessor is unavailable")
        ...
        for row, result in zip(staged_rows, released_results, strict=True):
            ...
            if cursor.rowcount != 1:
                raise Stage3AdvanceError("staged fact release CAS was lost")
...
def _advance_scope(context: WorkContext) -> UowScope:
    ...
    if (
        ...
        raise Stage3AdvanceError("invalid demo advance claim")
    ...
    if scope.process_role != "worker" or any(
        ...
        raise Stage3AdvanceError("demo advance claim scope drifted")
    ...
    if set(snapshot) != set(expected_snapshot) or any(
        ...
        raise Stage3AdvanceError("demo advance authority snapshot drifted")
    return scope
...
def _reconciliation_scope(context: WorkContext) -> UowScope:
    ...
    if (
        ...
        raise Stage3ReconciliationError("invalid date reconciliation claim")
    scope = context.store.uow_scope_for_claim(claim)
    expected_scope = {
        "namespace_id": claim.namespace_id,
        "namespace_generation": claim.namespace_generation,
...
def build_stage3_worker_handlers(
    ...
    if DEMO_ADVANCE_QUEUE in settings.worker_queues:
        ...
        handlers[DEMO_ADVANCE_QUEUE] = Stage3AdvanceHandler(
            cipher=cipher,
