...
class SleepApiAuthenticatorPort(Protocol):
    def authenticate(
        self,
        request: Request,
        *,
...
class SleepApiRuntime(Protocol):
    @property
    def authenticator(self) -> SleepApiAuthenticatorPort: ...

    def poll_events(self, identity: VerifiedActorIdentity, *, subject_id: str,
                    cursor: str | None, limit: int) -> EventPollResponse: ...
    def get_lifecycle(self, context: AuthenticatedActorContext, *,
                      subject_id: str) -> LifecycleResponse: ...
    def get_current_risk(self, context: AuthenticatedActorContext, *,
                         subject_id: str) -> CurrentRiskResponse: ...
    def list_night_episodes(self, context: AuthenticatedActorContext, *,
                            subject_id: str, limit: int,
                            cursor: str | None) -> NightEpisodePageResponse: ...
    def get_night_episode(self, context: AuthenticatedActorContext, *,
                          subject_id: str, night_episode_id: str,
                          revision_id: str | None) -> NightEpisodeResponse: ...
    def get_role_view(self, context: AuthenticatedActorContext, *,
                      subject_id: str, night_episode_id: str,
                      revision_id: str | None) -> RoleViewResponse: ...
    def get_operation(self, context: AuthenticatedActorContext, *,
                      operation_id: str) -> OperationStatusResponse: ...
    def require_subject(self, context: AuthenticatedActorContext,
                        subject_id: str) -> None: ...
    def submit_command(self, context: AuthenticatedActorContext, *,
                       operation_type: str, route_template: str,
                       target_resource_id: str | None, idempotency_key: str,
...
async def sleep_api_error_handler(
    ...
    if not isinstance(exc, (SleepApiSecurityError, SleepApiApplicationError)):
        raise TypeError("unexpected Sleep API exception")
...
async def sleep_api_validation_handler(
    ...
    if not isinstance(exc, RequestValidationError):
        raise TypeError("unexpected Sleep API validation exception")
