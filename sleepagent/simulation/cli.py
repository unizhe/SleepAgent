...
def verify_backend(
    ...
    *,
    ...
    restart_command_worker: bool = False,
    include_read_models: bool = False,
    include_effects: bool = False,
    expected_night_count: int = 1,
    ...
    if product_client is not None:
        ...
        if include_commands:
            command_result = verify_command_backend(
                product_client,
                subject_id=subject_id,
                ...
                restart_worker=(
                    restart_worker if restart_command_worker else None
                ),
            )
        if include_effects:
            if command_result is None:
                raise DemoCliError("effect verification requires command evidence")
            effect_result = verify_effects_reconciliation_backend(
                product_client,
                subject_id=subject_id,
                actor_ids=selected_actor_ids,
        ...
        if include_read_models:
            read_model_result = verify_read_models_backend(
                client,
                product_client,
                subject_id=subject_id,
                actor_ids=selected_actor_ids,
...
def verify_command_backend(
    ...
    *,
    ...
    """Prove command commands through only signed public HTTP surfaces."""

    if set(actor_ids) != {"elder", "family", "doctor"}:
        raise DemoCliError("command verifier requires exactly three Product actors")
    if not night_episode_id or not night_episode_revision_id:
        raise DemoCliError("command verifier requires the committed Episode identity")
    ...
    if care_before.get("items"):
        raise DemoCliError("command fresh scenario exposed Care before confirmation")
    ...
    def submit_product(
        ...
        *,
        ...
        key = idempotency_key or f"command-{name}-{uuid4()}"
        ...
        if status != 202 or value.get("state") != "accepted":
            raise DemoCliError(f"command {name} was not accepted")
        operation_id = str(value.get("operation_id") or "")
        if not operation_id:
            raise DemoCliError(f"command {name} omitted operation_id")
        ...
        *,
        ...
        while True:
            ...
            if state in {
                ...
                raise DemoCliError(f"command {name} ended as {state}")
            if time.monotonic() >= deadline:
                raise DemoCliError(f"command {name} did not reach {expected_state}")
            time.sleep(min(0.2, max(0.0, deadline - time.monotonic())))
    ...
    def submit_sleep(
        ...
        *,
        ...
        value, headers, status = client.signed_json(
            ...
            scope=(scope,),
            payload=payload,
            idempotency_key=f"command-{name}-{uuid4()}",
            accepted_statuses=(202,),
        )
        ...
        while True:
            ...
            if state in {"failed", "cancelled"}:
                raise DemoCliError(f"command {name} ended as {state}")
            if time.monotonic() >= deadline:
                raise DemoCliError(f"command {name} did not succeed")
            time.sleep(min(0.2, max(0.0, deadline - time.monotonic())))
    ...
    if replay_status != 202 or replayed.get("operation_id") != start_id:
        raise DemoCliError("command same-key replay did not return the original Operation")
    ...
    if conflict_status != 409 or conflict.get("code") != "idempotency_conflict":
        raise DemoCliError("command same-key/different-body did not conflict")
    ...
    if declined.get("care_action_id") is not None or declined.get(
        ...
        raise DemoCliError("command decline created a Care or delivery effect")
    ...
    if any(
        ...
        raise DemoCliError("command declined candidate leaked into /care")
    ...
    return {
        "schema_version": "backend_command_verification.v1",
        "verified": True,
        "data_mode": "replay",
        "synthetic_non_release": True,
...
def verify_effects_reconciliation_backend(
    ...
    *,
    ...
    if set(actor_ids) != {"elder", "family", "doctor"}:
        raise DemoCliError("effect verifier requires exactly three Product actors")
    care_action_id = _required_public_text(
        command_result.get("care_action_id"), "effect CareAction"
    )
    delivery_intent_id = _required_public_text(
        command_result.get("delivery_intent_id"), "effect delivery intent"
    )
    confirm_interaction_id = _required_public_text(
        command_result.get("confirm_interaction_id"), "effect confirm interaction"
    )
    decline_interaction_id = _required_public_text(
        command_result.get("decline_interaction_id"), "effect decline interaction"
    )
    deadline = time.monotonic() + wait_seconds
    while True:
        care = client.read_model(
        ...
        if len(active_actions) == 1 and not declined_actions:
            break
        if declined_actions:
            raise DemoCliError("effect decline created a Care delivery effect")
        if time.monotonic() >= deadline:
            raise DemoCliError("effect delivery effect was not publicly active")
        time.sleep(min(0.2, max(0.0, deadline - time.monotonic())))
    return {
        "schema_version": "backend_effects_verification.v1",
        "verified": True,
        "data_mode": "replay",
        "synthetic_non_release": True,
...
def verify_read_models_backend(
    demo_client: DemoHttpClient | Any,
    product_client: ProductHttpClient | Any,
    *,
    subject_id: str,
    ...
    if set(actor_ids) != {"elder", "family", "doctor"}:
        raise DemoCliError("read-model verifier requires exactly three Product actors")
    if expected_night_count < 1 or expected_night_count > 15:
        raise DemoCliError("read-model expected night count is out of bounds")
    if advance_seconds < 0 or advance_seconds > 604_800:
        raise DemoCliError("read-model advance seconds are out of bounds")
    ...
    if advance_seconds:
        ...
        key = f"read-model-advance-{uuid4()}"
        ...
        if replayed.get("operation_id") != advance_operation_id:
            raise DemoCliError("read-model advance replay changed Operation identity")
        ...
        if (
            ...
            raise DemoCliError("read-model committed ScenarioClock does not match receipt")
    ...
    while True:
        ...
        if time.monotonic() >= deadline:
            raise DemoCliError(
                "read-model read models did not reach the expected night count"
            )
        time.sleep(min(0.2, max(0.0, deadline - time.monotonic())))
    ...
    if expected_night_count > 1:
        for kind in ("trends", "records"):
            ...
            if not second["items"] or second["items"][0] == first["items"][0]:
                raise DemoCliError(f"read-model {kind} cursor repeated its first item")
            cursor_proofs[kind] = cursor

    return {
        "schema_version": "backend_read_models_verification.v1",
        "verified": True,
        "data_mode": "replay",
        "synthetic_non_release": True,
        "expected_night_count": expected_night_count,
...
def verify_bounded_retention_backend(
    ...
    *,
    ...
    baseline = verify_backend(
        ...
        expected_night_count=4,
        advance_seconds=604_800,
    )
    ...
    reset_key = f"retention-reset-{uuid4()}"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "verify":
            if args.restart_command_worker and not (
                args.commands
                or args.suite in {
                    "commands-interactions",
                    "effects-reconciliation",
                ...
                raise DemoCliError(
                    "--restart-command-worker requires commands-interactions"
                )
            if args.restart_command_worker and args.mode == "clean":
                raise DemoCliError(
                    "--restart-command-worker requires a restart verification mode"
                )
            if not args.service_credential or not args.actor_private_key:
                raise DemoCliError(
            ...
            include_commands = args.commands or args.suite in {
                "commands-interactions",
                "effects-reconciliation",
            }
            include_read_models = args.suite == "read-models-abnormal"
            include_effects = args.suite == "effects-reconciliation"
            if args.restart_command_worker and not include_commands:
                raise DemoCliError(
                    "--restart-command-worker requires commands-interactions"
                )
            if args.suite == "bounded-retention":
                ...
                result = verify_backend(
                    ...
                    restart_command_worker=args.restart_command_worker,
                    include_read_models=include_read_models,
                    include_effects=include_effects,
                    expected_night_count=args.expected_night_count,
