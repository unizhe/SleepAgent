...
-- Permit a claimed scheduled worker to reuse the frozen read-only
-- Perceptor Pull planner and durable ingress.  The original API/external-service
-- path remains unchanged; worker access is limited to the exact History and
-- SleepReport handler grants carried by the durable operation authority.
...
BEGIN
  IF NOT (
       ...
       OR (
         ...
         AND NULLIF(current_setting('sleepagent.purpose', TRUE), '') = 'worker'
       )
     )
  ...
  END IF;
  IF NULLIF(current_setting('sleepagent.process_role', TRUE), '') = 'worker'
     AND (
       resolved_surface NOT IN ('history', 'sleep_report')
       OR NOT EXISTS (
         SELECT 1
         FROM public.backend_service_principals AS scheduled_principal
         JOIN public.backend_principal_grants AS scheduled_grant
           ON scheduled_grant.principal_id = scheduled_principal.principal_id
          AND scheduled_grant.namespace_id = target_namespace_id
          AND scheduled_grant.data_mode = 'live'
          AND scheduled_grant.purpose = 'worker'
          AND scheduled_grant.authorization_epoch =
            current_setting(
              'sleepagent.authorization_epoch', TRUE
            )::BIGINT
          AND scheduled_grant.status = 'active'
          AND scheduled_grant.valid_from <= clock_timestamp()
          AND (
            scheduled_grant.valid_until IS NULL
            OR scheduled_grant.valid_until > clock_timestamp()
          )
          AND scheduled_grant.allowed_handlers_json ? CASE resolved_surface
            WHEN 'history' THEN 'perceptor.history_overlap_pull'
            WHEN 'sleep_report' THEN 'perceptor.sleep_report_pull'
          END
         WHERE scheduled_principal.principal_id = NULLIF(
             current_setting('sleepagent.service_principal_id', TRUE), ''
           )
           AND scheduled_principal.database_role_name::TEXT =
             session_user::TEXT
           AND scheduled_principal.principal_kind = 'worker'
           AND scheduled_principal.status = 'active'
       )
     ) THEN
    RAISE EXCEPTION 'scheduled Perceptor Pull worker authority mismatch';
  END IF;
  ...
  WHERE principal.principal_id = NULLIF(
    ...
    AND (
      ...
      OR (
        NULLIF(current_setting('sleepagent.process_role', TRUE), '') = 'worker'
        AND principal.principal_kind = 'worker'
        AND grant_row.allowed_handlers_json ?
