...
def _task_status_value(task: RadarAgentTask) -> str:
    return task.status.value


def _event_payload(event: RadarTaskEvent) -> dict[str, Any]:
    return event.model_dump(mode="json")


def _event_type(event: RadarTaskEvent) -> str:
...
def _format_sse_event(event: RadarTaskEvent) -> str:
    ...
    return (
        ...
        f"event: {payload.get('event_type', 'task.event')}\n"
