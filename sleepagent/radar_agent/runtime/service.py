...
class TaskService:
        ...
        with self._lock:
            ...
            if idempotency_key:
                ...
                if existing is not None:
                    ...
                    if not isinstance(existing, RadarAgentTask):
                        raise IdempotencyConflict(
                            "idempotency key belongs to a read-only historical task"
                        )
    ...
    ) -> RadarAgentTask | HistoricalTaskRecord:
    ...
    def _require_canonical_runtime(
        ...
        if not isinstance(task, RadarAgentTask):
            raise InvalidTaskTransition("historical task is read-only")
        return task
    ...
    def emit_event(
        ...
        *,
        ...
        with self._lock:
            ...
            if not isinstance(event, RadarTaskEvent):
                raise RuntimeError("canonical task event was decoded as historical data")
            return event
    ...
    def expire_confirmations(self, task_id: str, *, before: datetime) -> list[HumanConfirmationRequest]:
        ...
        pending = _blocking_pending(self.store.list_confirmations(task_id))
        current = self._require_canonical_runtime(self.get_task(task_id))
    ...
    def _next_event_sequence(self, task: RadarAgentTask) -> int:
        ...
        if not events:
            ...
            last_event = events[-1]
            if not isinstance(last_event, RadarTaskEvent):
                raise RuntimeError(
                    "canonical task event was decoded as historical data"
                )
            persisted_sequence = last_event.sequence
...
def _task_export_value(
    task: RadarAgentTask | HistoricalTaskRecord,
    field_name: str,
) -> Any:
    """Serialize shared task fields without executing historical DTO behavior."""
