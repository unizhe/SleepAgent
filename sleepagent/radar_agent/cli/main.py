...
def _write_pretty(trace: dict[str, Any], *, stdout: TextIO) -> None:
    ...
    if dynamic:
        ...
        if budget:
            print(
                "  budget: "
                f"plan={budget.get('initial_plan_count')}/{budget.get('initial_plan_limit')} "
                f"replan={budget.get('replan_count')}/{budget.get('replan_limit')} "
                f"agent={budget.get('capability_agent_call_count')}/{budget.get('capability_agent_call_limit')} "
                f"tool={budget.get('deterministic_tool_call_count')}/{budget.get('deterministic_tool_call_limit')} "
                f"model={budget.get('total_model_call_count')}/{budget.get('total_model_call_limit')} "
        ...
        for invocation in dynamic.get("model_invocations", []):
            print(
                ...
                f"{invocation.get('model_provider')}/{invocation.get('model_id')} "
                f"request={invocation.get('provider_request_id') or '-'} "
