...
_TRIGGER_ALIASES = {
    "watch_risk_signal": QuestionnaireTrigger.WATCH_OR_ESCALATE,
    "escalate_risk_signal": QuestionnaireTrigger.WATCH_OR_ESCALATE,
}
...
class QuestionnaireService:
    def __init__(
        self,
        *,
        banks: Iterable[QuestionnaireBank] | None = None,
        policies: Iterable[QuestionnairePolicy] | None = None,
        skill_packs: Iterable[SkillQuestionPack] | None = None,
        tone_rewriter: ToneRewriter | None = None,
    ) -> None:
        self.banks = list(banks) if banks is not None else [DEFAULT_QUESTIONNAIRE_BANK]
        self.policies = list(policies) if policies is not None else list(DEFAULT_QUESTIONNAIRE_POLICIES)
        self.skill_packs = list(skill_packs) if skill_packs is not None else [DEFAULT_SKILL_QUESTION_PACK]
    ...
    def select(
        ...
        *,
        subject_id: str,
        role: str,
        triggers: Iterable[QuestionnaireTrigger],
        prior_entries: Iterable[QuestionnaireEntry] = (),
        now: datetime | None = None,
        max_questions: int = 3,
        preferred_question_ids: Iterable[str] | None = None,
    ) -> QuestionnaireSelection:
        if role not in {"elder", "family", "doctor"}:
            raise ValueError("Micro-questionnaires are role-scoped to elder/family/doctor.")
        if max_questions < 1 or max_questions > 3:
            raise ValueError("Each micro-questionnaire turn must contain 1-3 questions.")
        selected_at = now or datetime.now(timezone.utc)
        active_triggers = set(triggers)
        preferred = list(dict.fromkeys(preferred_question_ids or []))
        entries = list(prior_entries)
        candidates: list[QuestionnaireCandidate] = []
        suppressed: list[str] = []
        for policy in self.policies:
            trigger = _policy_trigger(policy)
            if not policy.enabled or trigger not in active_triggers or role not in policy.applicable_roles:
                continue
            if _inside_cooldown(policy, subject_id, role, entries, selected_at):
                suppressed.append(policy.policy_id)
                continue
            limit = min(max_questions - len(candidates), policy.max_questions_per_turn)
            if limit <= 0:
                break
            policy_candidates = self._candidates_for_policy(policy, trigger, role)
            if preferred:
                preference_order = {
                    question_id: index
                    for index, question_id in enumerate(preferred)
                }
                policy_candidates = sorted(
                    (
                        candidate
                        for candidate in policy_candidates
                        if candidate.question_id in preference_order
                    ),
                    key=lambda candidate: preference_order[candidate.question_id],
                )
            for candidate in policy_candidates:
                if candidate.question_id in {item.question_id for item in candidates}:
                    continue
                candidates.append(candidate)
                if len(candidates) >= max_questions or limit <= 1:
                    break
                limit -= 1
        ...
        return QuestionnaireSelection(
            ...
            suppressed_policy_ids=list(dict.fromkeys(suppressed)),
        )
    ...
    def capture_answers(
        ...
        *,
        ...
        candidates = {item.question_id: item for item in selection.candidates}
        entries: list[QuestionnaireEntry] = []
        for answer in answers:
            candidate = candidates.get(answer.question_id)
            if candidate is None:
                raise ValueError("Answers must reference questions from the issued selection.")
            evidence_ref = (
                f"questionnaire:{selection.subject_id}:{candidate.question_id}:"
        ...
        return QuestionnaireCapture(entries=entries)
    ...
    def _candidates_for_policy(
        ...
        for bank in self.banks:
            if not bank.reviewed:
                continue
            for question_id, raw_question in bank.questions.items():
                question = _normalize_question(question_id, raw_question)
                candidate = self._candidate(question, policy, trigger, role, "bank", bank.bank_id, bank.version)
                if candidate is not None:
                    candidates.append(candidate)
        for pack in self.skill_packs:
            for question in pack.questions.values():
                candidate = self._candidate(question, policy, trigger, role, "skill", pack.skill_id, pack.version)
                if candidate is not None:
                    candidates.append(candidate)
        return candidates
    ...
    def _candidate(
        ...
        if policy.allowed_question_ids and question.question_id not in policy.allowed_question_ids:
            return None
        if role not in question.applicable_roles:
            return None
        if question.triggers and trigger.value not in question.triggers:
            return None
        text = question.role_text.get(role, question.text)
        if self.tone_rewriter is not None:
            text = self.tone_rewriter.rewrite_question(text=text, role=role).strip()
            if not text or len(text) > 120:
                raise ValueError("Tone rewriting must preserve one short question.")
...
def infer_questionnaire_triggers(
    context: ContextPacket,
    ledger: EvidenceLedger,
) -> list[QuestionnaireTrigger]:
    triggers: list[QuestionnaireTrigger] = []
    summaries = context.evidence_packet.night_summaries
    if summaries and summaries[-1].data_quality_status != RadarDataQualityStatus.GOOD:
        triggers.append(QuestionnaireTrigger.DATA_QUALITY_INSUFFICIENT)
    if (
        "trend" in (ledger.uncertainty or "").lower()
        or ledger.derived_metrics.get("trend_cause_known") is False
        or context.evidence_packet.data_quality.get("trend_cause_unknown") is True
    ):
        triggers.append(QuestionnaireTrigger.TREND_CAUSE_UNKNOWN)
    risk = str(ledger.derived_metrics.get("risk_level", RiskLevel.INFO.value))
    if risk in {RiskLevel.WATCH.value, RiskLevel.ESCALATE.value}:
        triggers.append(QuestionnaireTrigger.WATCH_OR_ESCALATE)
    question = str(context.evidence_packet.data_quality.get("user_question", "")).lower()
    if any(marker in question for marker in ("最近变差", "近期变差", "为什么最近", "why.*recent", "getting worse")):
        triggers.append(QuestionnaireTrigger.RECENT_WORSENING_QUESTION)
    if (
        context.task_context.role == "doctor"
        and not context.evidence_packet.supplementary_documents
    ) or context.evidence_packet.data_quality.get("doctor_background_missing") is True:
        triggers.append(QuestionnaireTrigger.DOCTOR_BACKGROUND_MISSING)
    return list(dict.fromkeys(triggers))


def attach_questionnaire_entries(
    ledger: EvidenceLedger,
    entries: Iterable[QuestionnaireEntry],
) -> EvidenceLedger:
    combined = list(ledger.questionnaire_entries)
    refs = list(ledger.canonical_evidence_refs)
    for entry in entries:
        if entry not in combined:
            combined.append(entry)
        if entry.evidence_ref and entry.evidence_ref not in refs:
            refs.append(entry.evidence_ref)
    metrics = dict(ledger.derived_metrics)
    metrics["questionnaire_entry_count"] = len(combined)
    return ledger.model_copy(
        update={
            "questionnaire_entries": combined,
...
def _normalize_question(
    question_id: str,
    question: str | QuestionnaireQuestion,
) -> QuestionnaireQuestion:
    if isinstance(question, QuestionnaireQuestion):
        return question
...
def _policy_trigger(policy: QuestionnairePolicy) -> QuestionnaireTrigger:
    if policy.trigger in _TRIGGER_ALIASES:
        return _TRIGGER_ALIASES[policy.trigger]
    return QuestionnaireTrigger(policy.trigger)
...
def _inside_cooldown(
    ...
    threshold = now - timedelta(hours=policy.cooldown_hours)
    return any(
        entry.subject_id == subject_id
        and entry.role == role
        and entry.policy_id == policy.policy_id
        and entry.collected_at >= threshold
        for entry in entries
    )
