...
1. `docs/audits/product-positioning/PLAN.md` 继续决定第一用户、非诊断边界和“晨间解释 + 个性化对话照护”产品主线。
2. `docs/audits/product-information-architecture/PLAN.md` 继续决定老人、家属、医生的用户入口与信息归属。
3. `docs/audits/agent-architecture/PLAN.md` 是 Agent 名单、职责、Episode、工具、权限、Safety、Memory 和发布权的上位规范；Skill 不得改变这些决定。
...
```text
sleepagent/product_runtime/skills/
├── contracts.py
├── loader.py
├── registry.py
├── resolver.py
...
Skill 选择必须服从下表；“mandatory”指该 Agent 一旦被本 Episode 调用就必须加载，不改变 `docs/audits/agent-architecture/PLAN.md` 对 Agent 是否必经的上位规定：
...
睡眠习惯能力不创建第十九个 Skill，也不创建画像专用 Agent。其权威逐跳路由见 [`../docs/product/habit-profile/HABIT-SKILL-TOOL-ROUTING.md`](../docs/product/habit-profile/HABIT-SKILL-TOOL-ROUTING.md)，有限决策缺口见 [`../docs/product/habit-profile/DECISION-GAP-MAPPING.md`](../docs/product/habit-profile/DECISION-GAP-MAPPING.md)，在线事件与 Safety/Care Delivery 合同见 [`../docs/product/habit-profile/ONLINE-REASONING-FREEZE.md`](../docs/product/habit-profile/ONLINE-REASONING-FREEZE.md)。现有 Skill 的冻结职责如下：
