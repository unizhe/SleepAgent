...
| `sleepagent/persistence/uow.py` | connection provider、UoW factory、UoW-scoped repository factory |
| `sleepagent/persistence/migrate.py` | 独立 migration apply/check CLI；应用与 Worker 不执行 DDL |
...
1. `sleepagent/product_runtime` 是唯一 canonical 四角色实现；旧 LangGraph runtime 不进入新依赖图。
