...
- [产品定位计划](../docs/audits/product-positioning/PLAN.md)：第一用户、双主线、照护循环、验收与非诊断边界。
- [产品定位审查记录](../docs/audits/product-positioning/PLAN-REVIEW-LOG.md)：定位锁定过程。
- [产品信息架构](../docs/audits/product-information-architecture/PLAN.md)：目标“今天、趋势、问问 SleepAgent、记录”四页主线。

### 1+2+1 Agent

- [Agent 架构计划](../docs/audits/agent-architecture/PLAN.md)：四角色责任、最小路径、状态、权限、Safety 和 Skill 边界。
- [Agent 合同](../sleepagent/product_runtime/contracts.py)：四个 `AgentId`、SourceScope、FactSnapshot、工作成果和 Receipt。
- [Agent 与 Tool 注册表](../sleepagent/product_runtime/registry.py)：角色定义、调用关系和 deny-by-default allowlist。
- [ProductEpisodeRunner](../sleepagent/product_runtime/runner.py)：统一运行、Habit 融入、Safety、确认、提交与归纳。
- [Agent 构建与验证日志](../docs/audits/agent-architecture/PLAN-REVIEW-LOG.md)：从设计收口到生产接线和持久化的阶段记录。
- [当前验收 Manifest](../docs/audits/agent-architecture/ACCEPTANCE-MANIFEST.json)：当前版本身份与正式 evidence 空缺。

### 睡眠习惯

- [睡眠习惯画像计划](../docs/product/habit-profile/PLAN.md)：四类上下文、渐进采集、确认、冲突和角色边界。
- [v1 概念覆盖](../docs/product/habit-profile/V1-CONCEPT-COVERAGE.md)：完整冻结范围、ObjectiveBaseline 和 17 个待补概念。
- [Habit Skill/Tool 路由](../docs/product/habit-profile/HABIT-SKILL-TOOL-ROUTING.md)：Habit 如何融入四角色。
- [Decision Gap 映射](../docs/product/habit-profile/DECISION-GAP-MAPPING.md)：何时允许提问，以及回答能改变什么决策。
- [默认 Habit 目录](../sleepagent/product_runtime/questionnaire/defaults.py)：当前 10 个可执行概念。
- [Habit Profile 合同](../sleepagent/product_runtime/habit_profile.py)：ObjectiveBaseline、Fact、ChangeSet、Confirmation 和 Store。
- [Habit 产品应用](../sleepagent/product_runtime/habit_application.py)：可选建档、回答、候选、确认、更正与遗忘。
- [Habit 验收矩阵](../docs/product/habit-profile/ACCEPTANCE-MATRIX.md)：28 项自动化标准及正式 release gate。
...
- [纵向记忆代码](../sleepagent/product_runtime/longitudinal_memory.py)：GovernedMemory、EpisodeDigest、Manifest、Worker 和查询服务。
